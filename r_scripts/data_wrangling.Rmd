...
dr_variants_pivot <- reshape2::dcast(dr_variants, change ~ ., value.var = "wgs_id", fun.aggregate = length)
names(dr_variants_pivot) <- c("Change", "Freq")
...
The `r n_samps_total` samples were previously published between `r min(pub_years)` and `r max(pub_years)` in `r length(unique(metadata$study_name))` different studies. 
...
- [DR] SNPs can vary between MTB lineages therefore local data is required to describe different strain populations.
...
knitr::kable(dr_variants_results_table, caption = sprintf("Known drug resistance-associated mutations by gene and drug (n=%s)", n_samps_total))
