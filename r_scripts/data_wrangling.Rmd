...
# colnames(dist) %in% dated_samps
# 
# 
# dated_samps %in% colnames(dist)
# 
# 
# "ERR5897118" %in% colnames(dist)
# 
# 
# sort(colnames(dist))
...
year_tab <- reshape2::dcast(metadata, year ~ "N", value.var = id_col, fun.aggregate = length)
...
table_1 <- rbind(table_1, 
                 ...
                 setNames(dr_tab[, c("Drug", "N genotypic resistant", "%")], names(table_1)), 
...
```{r ggtree-all-samps, echo=FALSE, out.width = "1500px", warning = F, message = F, cache=F, fig.show="hold", fig.cap=""}
...
dr_status_hm <- lin_hm + ggnewscale::new_scale_fill()
