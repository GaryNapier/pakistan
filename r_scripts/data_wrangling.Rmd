...
discrepancy_data$id_year <- row.names(discrepancy_data) # Put id back in for the other variants stuff
...
for(i in seq(FN_drugs_split)){
  ...
  FN_samps <- discrepancy_data[discrepancy_data[drug] == FN, "id_year"]
  # Subset the other variants data by the samples
  ov_sub <- other_variants[other_variants[,"id_year"] %in% FN_samps, ]
...
# other_variants[other_variants[, "id_year"] == "ERR3335766_2016", ]
 
# discrepancy_data[discrepancy_data[, "id_year"] == "ERR3335766_2016", ]
...
for(i in seq(FP_drugs_split)){
  ...
  drug_proper <- gsub("_test", "", drug)
  # Get the FN samples for the drug
  FP_samps <- discrepancy_data[discrepancy_data[drug] == FP, "id_year"]
  # Subset the other variants data by the samples
  ov_sub <- other_variants[other_variants[,"id_year"] %in% FP_samps, ]
  # Get the loci for the relevant drug
  loci <- dr_genes[dr_genes[, "drugs"] == drug_proper, ]$locus
  # Subset the FN other variants data by the loci associated with the drug  
  ov_sub <- ov_sub[ov_sub[, "locus_tag"] %in% loci, ]
