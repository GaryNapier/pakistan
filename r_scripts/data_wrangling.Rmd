...
Date of collection was recorded between the years `r first_year` and `r last_year` (`r last_year - first_year` years) for `r sum(!(is.na(metadata$year)))` samples, which represent probable closely-related, if not potiential transmission cases. A dated phylogenetic tree was constructed with these samples in order to infer clusters of closely related samples. 
