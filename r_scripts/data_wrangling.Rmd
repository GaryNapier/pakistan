...
# Separate into dataframes where there is / is no overlap in samples 
# (at least one sample has more than one mutation in that gene)
mutations_split_single <- list()
mutations_split_multiple <- list()
for(i in seq(mutations_split)){
  if(any(duplicated(mutations_split[[i]]$wgs_id))){
    mutations_split_multiple[[i]] <- mutations_split[[i]]
  }else{
    mutations_split_single[[i]] <- mutations_split[[i]]
  }
}

# Remove NULL
mutations_split_single <- mutations_split_single[lengths(mutations_split_single) != 0]
mutations_split_multiple <- mutations_split_multiple[lengths(mutations_split_multiple) != 0]

# Split multiple on mutation
mutations_split_multiple <- lapply(mutations_split_multiple, function(x){
  split(x, x$mutation)
})

# Merge with rest of samples to get NA - i.e. no colour where no mutation
# nusg_mut <- merge(metadata["wgs_id"], nusg_mut, by = "wgs_id", all.x = T, sort = F)
mutations_split_single <- lapply(mutations_split_single, function(x) {
  merge(metadata["wgs_id"], x, by = "wgs_id", all.x = T, sort = F)
})
# Multiple
for(i in seq(mutations_split_multiple)){
  for(j in seq(mutations_split_multiple[[i]])){
    mutations_split_multiple[[i]][[j]] <- merge(metadata["wgs_id"],
                                                mutations_split_multiple[[i]][[j]], 
...
# for(i in seq(mut_cols)){
#   mut_cols[[i]][length(mut_cols[[i]])] <- "#FFFFFF"
# }
...
# for(i in seq(mut_cols)){
#   names(mut_cols[[i]]) <- unique(mutations_split[[i]]$mutation)
# }
for(i in seq(mutations_cols)){
  names(mutations_cols[[i]]) <- c(unique(mutations_split[[i]]$mutation), NA)
}

# Assign ids as row names for the ggtrees
# row.names(nusg_mut) <- nusg_mut$wgs_id
# for(i in seq(mutations_split)){
#   row.names(mutations_split[[i]]) <- mutations_split[[i]]$wgs_id
# }
for(i in seq(mutations_split_single)){
  row.names(mutations_split_single[[i]]) <- mutations_split_single[[i]]$wgs_id
}
for(i in seq(mutations_split_multiple)){
  for(j in seq(mutations_split_multiple[[i]])){
  row.names(mutations_split_multiple[[i]][[j]]) <- mutations_split_multiple[[i]][[j]]$wgs_id
  }
  }
...
for(i in seq(mutations_split_single)){
  mutations_split_single[[i]] <- mutations_split_single[[i]][!(names(mutations_split_single[[i]]) %in% "wgs_id")]
}
for(i in seq(mutations_split_multiple)){
  for(j in seq(mutations_split_multiple[[i]])){
    mutations_split_multiple[[i]][[j]] <- mutations_split_multiple[[i]][[j]][!(names(mutations_split_multiple[[i]][[j]]) %in% "wgs_id")]
  }}
...
gheatmap(trans_tree, mutations_split_single[[1]],
         width = width,
         offset = 0.003,
         color = NULL,
         colnames_position = "top",
         ...
         hjust = 0,
         font.size = 3) +
  scale_fill_manual(values = mutations_cols[[1]], breaks = names(mutations_cols[[1]]) ) +
  labs(fill = names(mutations_cols[1]))+
  legend_spec
