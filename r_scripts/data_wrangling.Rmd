...
- Evidence of transmission among
  - lineages 2 and 4 
  - Pre-MDR and MDR 
  - Peshawar
  
- Previously unknown DR mutations for drugs XXX in genes XXX
...
```
-->
```{r dist-dens-all-samples, echo=FALSE, fig.show="hold", out.width="50%", fig.cap = sprintf("Density of pairwise SNP distances for all samples (n=%s)", n_samps_total)}
...
```{r distanceqgraph, echo=FALSE, out.width = "2000px", fig.cap = sprintf("Connections among samples with <= %s SNP distance; n = %s. Thicker lines indicate smaller SNP distance.", threshold, length(attributes(dist_thresh)$Labels)), fig.show="hold"}
...
```{r trans-category-plots, echo=FALSE, out.width = "2000px", fig.cap = sprintf("Proportions of samples within %s SNPs (top) and size of residuals in chi-square tests for lineage, drug resistance status and location.", threshold), fig.show="hold"}
...
balloonplot(t(loc_contingency_tab), 
            ...
            xlab ="", ylab="",
            label = T, show.margins = FALSE, show.zeros = T, colsrt = 90)
corrplot(trans_loc_chi_sq$residuals, is.cor = FALSE)
